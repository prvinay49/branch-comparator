...
def sysLogger():
    ...
    logger.debug('syslogger initialized ')
...
class Automerger2:
    ...
    def processchangeIds(self):
        for index in self.sortedIssueQueue:
            ...
            try:
                ...
                temp = []
                for i in range(len(change_id_gerrits)):
                    changeInTarget = okr_automation.check_already_in_target(change_id_gerrits[i])
                    print('changes in ', changeInTarget)
                    if changeInTarget == 'Not available' :
                        print('cherry pick ')
                        #okr_automation.cherry_pick(change_id_gerrits[i])
                    else:
                        print('skip')
                        iterator = iterator +1
                        temp.append(self.get_gerrit_topic(change_id_gerrits[i]['change_id'])['topic_gerrit']['topic'])
    ...
    def check_already_in_target(self, tgt):
        pre_check = self.gerrit.get('/changes/?q=branch:%s+status:merged+%s&o=CURRENT_REVISION&o=CURRENT_COMMIT&o=MESSAGES'
        ...
        for issue in issuesDict:
            ...
            if str(issue.fields.issuetype) == 'Bug' or  str(issue.fields.issuetype) == 'Task' :
                    if str(issue.fields.resolution) == 'RM Approved':
                        issuesListfromFilter.append(issue.key)
